...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
  ...
  for (unsigned int y=0; y<blocked_grid[0].size(); ++y) {
    for (unsigned int x=0; x<blocked_grid.size(); ++x) {
      if (blocked_grid[x][y] == PATH || ( x==0 && y==0 ) ) {
         std::cout << "(" << x << "," << y << ") ";
      }
    }
  }
...
int moveFrom(int x, int y, std::vector<std::vector<GRID_STATUS> > grid, int start_x, int start_y) {
    if(x == 0 && y == 0) {
      print_grid(grid, start_x, start_y);
      return 1;
    }
    else if( x < 0 || y < 0) {
        return 0;
    ...
    grid[x][y] = PATH;
    return (moveFrom(x-1, y, grid, start_x, start_y) + moveFrom(x, y-1, grid, start_x, start_y));
}
...
int main(int argc, char* argv[]) {
  ...
  int c=moveFrom(start_x, start_y, blocked_grid, start_x, start_y);
