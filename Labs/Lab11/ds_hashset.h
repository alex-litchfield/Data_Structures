...
template <class KeyType, class HashFunc> class ds_hashset {
private:
  ...
  private:
    ...
    void prev() {
      ...
      if (m_list_itr != m_hs->m_table[m_index].begin())
        ...
        for (--m_index; m_index >= 0 && m_hs->m_table[m_index].empty();
             --m_index) {
        }
...
public:
  ...
  iterator find(const KeyType &key) {
    ...
    unsigned int index = hash_value % m_table.size();
    hash_list_itr p =
        std::find(m_table[index].begin(), m_table[index].end(), key);
  ...
  iterator begin() {
    ...
    for (unsigned int x = 0; x < m_table.size(); ++x) {

      if (m_table[x].size() > 0) {
        KeyType key = *(m_table[x].begin());
