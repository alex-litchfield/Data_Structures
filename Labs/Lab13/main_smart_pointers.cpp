...
public:
  ...
  Balloon(const std::string& name_) : name(name_) {
    ...
    num_ropes = 0;
    ropes = new Balloon*[MAX_NUM_ROPES];
    smart_ropes = new dsSharedPtr<Balloon>[MAX_NUM_ROPES];
  }
  ~Balloon() {
  ...
  void addRope(const dsSharedPtr<Balloon> & b) {
    smart_ropes[num_ropes] = b;
    num_ropes++;
  }
...
private:
  ...
  Balloon** ropes;
  dsSharedPtr<Balloon>* smart_ropes;
};


// ====================================================
// ====================================================
void deleteRecursive(std::set<Balloon*> & nodes, Balloon * b) {
  bool success;
  for (int i = 0; i < b->numRopes(); i++) {
    success = nodes.insert(b->getRope(i)).second;
    if (success) deleteRecursive(nodes, b->getRope(i));
  }
}
...
void deleteAll(Balloon * b) {
  ...
  Balloon * temp = b;
  deleteRecursive(nodes, temp);
  std::cout << nodes.size() << std::endl;
  for (std::set<Balloon*>::iterator i = nodes.begin(); i != nodes.end(); i++) {
    delete *i;
  }
}
...
delete elaine;
  ...
  henry2->printSmart();
  georgette2->printSmart();
  isabelle2->printSmart();*/
