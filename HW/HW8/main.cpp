...
void StudentTests(){
	...
	char c;
	assert(r1.index(0,c) && c == s1[0] && "MISMATCH BETWEEN A CHARACTER IN ROPE AND STRING USING INDEX");
	std::cout << "***PASSED INDEX TEST: FIRST VALUE OF A NODE" << std::endl;
	assert(r1.index(3,c) && c == s1[3] && "MISMATCH BETWEEN A CHARACTER IN ROPE AND STRING USING INDEX");
	std::cout << "***PASSED INDEX TEST: A MIDDLE VALUE OF A NODE" << std::endl;
	assert(r1.index(6,c) && c == s1[6] && "MISMATCH BETWEEN A CHARACTER IN ROPE AND STRING USING INDEX");
	std::cout << "***PASSED INDEX TEST: LAST VALUE OF A NODE" << std::endl;
}
